...
const PasswordReset = () => {
  ...
  useEffect(() => {
    ...
    const validateToken = async () => {
      try {
        const response = await axios.get(`${url}/api/users/validate-reset-token/${token}`);
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${url}/api/users/reset-password`, { token, newPassword: data.newPassword });
